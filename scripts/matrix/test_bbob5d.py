...
def place_legend_below(
    ax, handles, labels, show=True, legend_nrow=1, start_legend=3, loc_y=-0.11
):
    box = ax.get_position()
    ...
    if show:
        ax.legend(
            ...
            loc="upper center",
            fontsize=13,
            bbox_to_anchor=(start_legend, loc_y),
            fancybox=True,
            shadow=True,
            ncol=np.ceil(len(labels) / legend_nrow),
        )
