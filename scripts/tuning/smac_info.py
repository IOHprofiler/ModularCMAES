...
def plot_convergence(fid: int, dim: int, config_rt: dict[str, list[float]]):
    ...
    sorted_rts = np.array([ert(config_rt[str(x)]) for x in sorted_keys])
    mins = list(accumulate(sorted_rts, min))
    min_idx = np.array(
        [
            i
            for i, (rt, m) in enumerate(zip(sorted_rts, mins))
...
if __name__ == "__main__":
    ...
    for folder in sorted(os.listdir(args.root)):
        if not args.all and not folder.endswith(arg_folder):
            continue
        algf, *_ = os.listdir(os.path.join(args.root, folder))
        file_search = glob.glob(f"./{args.root}/{folder}/{algf}/*/runhistory.json")
        if len(file_search) != 1:
            print("file not found")
            break
        filename, *_ = file_search
