...
template <typename RNG> Float random_double()
{
    static RNG gen;
...
void define_options(py::module &main)
{
    ...
    py::native_enum<RecombinationWeights>(m, "RecombinationWeights", "enum.Enum")
    ...
    py::native_enum<BaseSampler>(m, "BaseSampler", "enum.Enum")
        .value("UNIFORM", BaseSampler::UNIFORM)
        .value("SOBOL", BaseSampler::SOBOL)
        .value("HALTON", BaseSampler::HALTON)
        .finalize();

    py::native_enum<SampleTranformerType>(m, "SampleTranformerType", "enum.Enum")
    ...
    py::native_enum<Mirror>(m, "Mirror", "enum.Enum")
        .value("NONE", Mirror::NONE)
        .value("MIRRORED", Mirror::MIRRORED)
        .value("PAIRWISE", Mirror::PAIRWISE)
        .finalize();

    py::native_enum<StepSizeAdaptation>(m, "StepSizeAdaptation", "enum.Enum")
    ...
    py::native_enum<CorrectionMethod>(m, "CorrectionMethod", "enum.Enum")
    ...
    py::native_enum<RestartStrategyType>(m, "RestartStrategy", "enum.Enum")
    ...
    py::native_enum<MatrixAdaptationType>(m, "MatrixAdaptationType", "enum.Enum")
    ...
    py::native_enum<CenterPlacement>(m, "CenterPlacement", "enum.Enum")
        ...
        .value("NOVELTY_WEIGHTED", CenterPlacement::NOVELTY_WEIGHTED)
        .value("MAXIMIN_TABOO", CenterPlacement::MAXIMIN_TABOO)
        .finalize();

    py::native_enum<RepellingType>(m, "RepellingType", "enum.Enum")
        .value("NONE", RepellingType::NONE)
        .value("COVERAGE", RepellingType::COVERAGE)
        .value("ADAPTIVE", RepellingType::ADAPTIVE)
        .finalize();
}
...
struct PySampler : sampling::Sampler
{
    ...
    PySampler(size_t d, std::function<Float()> f) :
        Sampler::Sampler(d),
        func(f)
    {
    }
...
void define_samplers(py::module &main)
{
    ...
    py::class_<CachedSampler, Sampler, std::shared_ptr<CachedSampler>>(m, "CachedSampler")
        .def(py::init<const std::shared_ptr<Sampler>>(), py::arg("sampler"))
        .def(py::init<std::vector<Vector>, bool>(),
             py::arg("cache"),
             py::arg("transform_ppf") = false)
    ...
    py::class_<SampleTransformer, Sampler, std::shared_ptr<SampleTransformer>>(m,
                                                                               "SampleTransformer")
        .def("raw", &SampleTransformer::raw);

    py::class_<IdentityTransformer, SampleTransformer, std::shared_ptr<IdentityTransformer>>(
        m, "IdentityTransformer")
        .def(py::init<const std::shared_ptr<Sampler>>(), py::arg("sampler"))
    ...
    py::class_<GaussianTransformer, SampleTransformer, std::shared_ptr<GaussianTransformer>>(
        m, "GaussianTransformer")
        .def(py::init<const std::shared_ptr<Sampler>>(), py::arg("sampler"))
    ...
    py::class_<LaplaceTransformer, SampleTransformer, std::shared_ptr<LaplaceTransformer>>(
        m, "LaplaceTransformer")
        .def(py::init<const std::shared_ptr<Sampler>>(), py::arg("sampler"))
    ...
    py::class_<LogisticTransformer, SampleTransformer, std::shared_ptr<LogisticTransformer>>(
        m, "LogisticTransformer")
        .def(py::init<const std::shared_ptr<Sampler>>(), py::arg("sampler"))
    ...
    py::class_<CauchyTransformer, SampleTransformer, std::shared_ptr<CauchyTransformer>>(
        m, "CauchyTransformer")
        .def(py::init<const std::shared_ptr<Sampler>>(), py::arg("sampler"))
    ...
    py::class_<DoubleWeibullTransformer,
               SampleTransformer,
               std::shared_ptr<DoubleWeibullTransformer>>(m, "DoubleWeibullTransformer")
        .def(py::init<const std::shared_ptr<Sampler>>(), py::arg("sampler"))
...
void define_utils(py::module &main)
{
    ...
    m.def("random_uniform",
          &random_double<rng::uniform<Float>>,
          "Generate a uniform random number in [0, 1]");
    m.def("random_normal",
          &random_double<rng::normal<Float>>,
...
void define_center_placement(py::module &main)
{
    ...
    py::class_<X0, Placement, std::shared_ptr<X0>>(m, "X0").def(py::init<>());

    py::class_<Uniform, Placement, std::shared_ptr<Uniform>>(m, "Uniform").def(py::init<>());

    py::class_<Zero, Placement, std::shared_ptr<Zero>>(m, "Zero").def(py::init<>());

    py::class_<Center, Placement, std::shared_ptr<Center>>(m, "Center").def(py::init<>());

    py::class_<MaximinTaboo, Placement, std::shared_ptr<MaximinTaboo>>(m, "MaximinTaboo")
        .def(py::init<>());

    py::class_<NoveltyWeighted, Placement, std::shared_ptr<NoveltyWeighted>>(m, "NoveltyWeighted")
        .def(py::init<>());
}
...
void define_repelling(py::module &main)
{
    ...
    py::class_<TabooPoint>(m, "TabooPoint")
        ...
        .def("shares_basin",
             &TabooPoint::shares_basin,
             py::arg("objective"),
             py::arg("xi"),
             py::arg("p"))
        .def("calculate_criticality", &TabooPoint::calculate_criticality, py::arg("p"))
        .def("clamp_radius", &TabooPoint::clamp_radius)

        .def_readwrite("solution", &TabooPoint::solution)
        .def_readwrite("radius", &TabooPoint::radius)
        .def_readwrite("min_radius", &TabooPoint::min_radius)
        .def_readwrite("max_radius", &TabooPoint::max_radius)
        ...
        .def_readwrite("duplicate_evaluations", &TabooPoint::duplicate_evaluations)
        .def_readwrite("rejected_count", &TabooPoint::rejected_count)
        .def_readwrite("checked_count", &TabooPoint::checked_count)
        .def_readwrite("last_rejection_rate", &TabooPoint::last_rejection_rate)

        .def("__repr__", [](const TabooPoint &tb) {
            return "<TabooPoint n_rep: " + std::to_string(tb.n_rep) +
                   " y: " + std::to_string(tb.solution.y) +
                   " radius: " + std::to_string(tb.radius) +
                   " criticality: " + std::to_string(tb.criticality) + ">";
        });
    ...
    py::class_<Repelling, std::shared_ptr<Repelling>>(m, "Repelling")
        ...
        .def("adapt_radii_after_sampling", &Repelling::adapt_radii_after_sampling, py::arg("p"))
    ...
    py::class_<CoverageRepelling, Repelling, std::shared_ptr<CoverageRepelling>>(
        m, "CoverageRepelling")
        .def(py::init<>());

    py::class_<AdaptiveRepelling, Repelling, std::shared_ptr<AdaptiveRepelling>>(
        m, "AdaptiveRepelling")
        .def(py::init<>())

        .def("initial_radius", &AdaptiveRepelling::initial_radius, py::arg("p"))
        .def("min_radius", &AdaptiveRepelling::min_radius, py::arg("p"))
        .def("max_radius", &AdaptiveRepelling::max_radius, py::arg("p"))

        .def_readwrite("initial_radius_factor", &AdaptiveRepelling::initial_radius_factor)
        .def_readwrite("min_radius_factor", &AdaptiveRepelling::min_radius_factor)
        .def_readwrite("max_radius_factor", &AdaptiveRepelling::max_radius_factor)

        .def_readwrite("shrink_factor", &AdaptiveRepelling::shrink_factor)

        .def_readwrite("target_rejection_rate", &AdaptiveRepelling::target_rejection_rate)
        .def_readwrite("max_rejection_rate", &AdaptiveRepelling::max_rejection_rate)

        .def_readwrite("last_restart_evaluations", &AdaptiveRepelling::last_restart_evaluations)
        .def_readwrite("average_restart_evaluations",
                       &AdaptiveRepelling::average_restart_evaluations)
        .def_readwrite("restart_eval_alpha", &AdaptiveRepelling::restart_eval_alpha)
        .def_readwrite("grow_eta", &AdaptiveRepelling::grow_eta)
        .def_readwrite("max_waste_ratio", &AdaptiveRepelling::max_waste_ratio)

        .def_readwrite("local_target_rejection_rate",
                       &AdaptiveRepelling::local_target_rejection_rate)
        .def_readwrite("local_max_rejection_rate", &AdaptiveRepelling::local_max_rejection_rate)
        .def_readwrite("min_checks_before_local_shrink",
                       &AdaptiveRepelling::min_checks_before_local_shrink);
    ...
    m.def("hill_valley_test",
          &distance::hill_valley_test,
          py::arg("u"),
          py::arg("v"),
          py::arg("f"),
          py::arg("n_evals"));
...
void define_matrix_adaptation(py::module &main)
{
    ...
    py::class_<Adaptation, std::shared_ptr<Adaptation>>(m, "Adaptation")
        ...
        .def("adapt_evolution_paths",
             &Adaptation::adapt_evolution_paths,
             py::arg("pop"),
             py::arg("weights"),
             py::arg("stats"),
             py::arg("settings"),
             py::arg("mu"),
             py::arg("lamb"))
        .def("adapt_evolution_paths_innner",
             &Adaptation::adapt_evolution_paths_inner,
             py::arg("pop"),
             py::arg("weights"),
             py::arg("stats"),
             py::arg("settings"),
    ...
    py::class_<None, Adaptation, std::shared_ptr<None>>(m, "NoAdaptation")
        .def(py::init<size_t, Vector, Float>(),
             py::arg("dimension"),
             py::arg("x0"),
    ...
    py::class_<CovarianceAdaptation, Adaptation, std::shared_ptr<CovarianceAdaptation>>(
        m, "CovarianceAdaptation")
        .def(py::init<size_t, Vector, Float>(),
             py::arg("dimension"),
             py::arg("x0"),
             py::arg("expected_length_z"))
        ...
        .def("adapt_covariance_matrix",
             &CovarianceAdaptation::adapt_covariance_matrix,
             py::arg("weights"),
             py::arg("modules"),
             py::arg("population"),
             py::arg("mu"))
        .def("perform_eigendecomposition",
             &CovarianceAdaptation::perform_eigendecomposition,
             py::arg("stats"))
    ...
    py::class_<SeparableAdaptation, Adaptation, std::shared_ptr<SeparableAdaptation>>(
        m, "SeparableAdaptation")
        .def(py::init<size_t, Vector, Float>(),
             py::arg("dimension"),
             py::arg("x0"),
             py::arg("expected_length_z"))
    ...
    py::class_<MatrixAdaptation, Adaptation, std::shared_ptr<MatrixAdaptation>>(m,
                                                                                "MatrixAdaptation")
        .def(py::init<size_t, Vector, Float>(),
             py::arg("dimension"),
             py::arg("x0"),
    ...
    py::class_<CholeskyAdaptation, Adaptation, std::shared_ptr<CholeskyAdaptation>>(
        m, "CholeskyAdaptation")
        .def(py::init<size_t, Vector, Float>(),
             py::arg("dimension"),
             py::arg("x0"),
             py::arg("expected_length_z"))
    ...
    py::class_<CovarianceNoEigvAdaptation,
               CovarianceAdaptation,
               std::shared_ptr<CovarianceNoEigvAdaptation>>(m, "CovarianceNoEigvAdaptation");

    py::class_<NaturalGradientAdaptation, Adaptation, std::shared_ptr<NaturalGradientAdaptation>>(
        m, "NaturalGradientAdaptation")
        .def(py::init<size_t, Vector, Float, Float>(),
             py::arg("dimension"),
             py::arg("x0"),
             py::arg("expected_length_z"),
             py::arg("sigma"))
        ...
        .def("compute_gradients",
             &NaturalGradientAdaptation::compute_gradients,
             py::arg("pop"),
             py::arg("weights"),
             py::arg("stats"),
             py::arg("settings"),
...
void define_parameters(py::module &main)
{
    ...
    py::class_<Modules>(m, "Modules")
        ...
        .def_readwrite("repelling_type", &Modules::repelling_type)
        ...
        .def("__repr__", [](Modules &mod) { return to_string(mod); });
    ...
    using AdaptationType =
        std::variant<std::shared_ptr<matrix_adaptation::MatrixAdaptation>,
    ...
    py::class_<mapping::CoordinateMapping, std::shared_ptr<mapping::CoordinateMapping>>(
        m, "CoordinateMapping")
        ...
        .def_property(
            ...
            [](Parameters &self) -> AdaptationType {
                ...
                case MatrixAdaptationType::MATRIX:
                    return std::dynamic_pointer_cast<matrix_adaptation::MatrixAdaptation>(
                        self.adaptation);
                ...
                case MatrixAdaptationType::SEPARABLE:
                    return std::dynamic_pointer_cast<matrix_adaptation::SeparableAdaptation>(
                        self.adaptation);
                case MatrixAdaptationType::CHOLESKY:
                    return std::dynamic_pointer_cast<matrix_adaptation::CholeskyAdaptation>(
                        self.adaptation);
                case MatrixAdaptationType::CMSA:
                    return std::dynamic_pointer_cast<matrix_adaptation::SelfAdaptation>(
                        self.adaptation);
                case MatrixAdaptationType::COVARIANCE_NO_EIGV:
                    return std::dynamic_pointer_cast<matrix_adaptation::CovarianceNoEigvAdaptation>(
                        self.adaptation);
                case MatrixAdaptationType::NATURAL_GRADIENT:
                    return std::dynamic_pointer_cast<matrix_adaptation::NaturalGradientAdaptation>(
                        self.adaptation);
                default:
                case MatrixAdaptationType::COVARIANCE:
                    return std::dynamic_pointer_cast<matrix_adaptation::CovarianceAdaptation>(
                        self.adaptation);
                }
            },
...
void define_bounds(py::module &main)
{
    ...
    py::class_<BoundCorrection, std::shared_ptr<BoundCorrection>>(m, "BoundCorrection")
        ...
        .def("correct", &BoundCorrection::correct, py::arg("index"), py::arg("parameters"))
        .def("correct_x",
             &BoundCorrection::correct_x,
             py::arg("xi"),
             py::arg("oob"),
             py::arg("sigma"),
             py::arg("settings"))
        .def("delta_out_of_bounds",
             &BoundCorrection::delta_out_of_bounds,
             py::arg("xi"),
             py::arg("oob"),
             py::arg("settings"))
        .def("is_out_of_bounds",
             &BoundCorrection::is_out_of_bounds,
             py::arg("xi"),
             py::arg("settings"));
    ...
    py::class_<Mirror, BoundCorrection, std::shared_ptr<Mirror>>(m, "Mirror").def(py::init<>());

    py::class_<UniformResample, BoundCorrection, std::shared_ptr<UniformResample>>(
        m, "UniformResample")
        .def(py::init<size_t>(), py::arg("dim"));
...
void define_mutation(py::module &main)
{
    ...
    py::class_<ThresholdConvergence, std::shared_ptr<ThresholdConvergence>>(m,
        ...
        .def("scale",
             &ThresholdConvergence::scale,
             py::arg("population"),
             py::arg("diameter"),
             py::arg("budget"),
             py::arg("evaluations"));

    py::class_<NoThresholdConvergence,
               ThresholdConvergence,
               std::shared_ptr<NoThresholdConvergence>>(m, "NoThresholdConvergence")
        .def(py::init<>());
    ...
    py::class_<Strategy, std::shared_ptr<Strategy>>(m, "Strategy")
        .def(py::init<std::shared_ptr<ThresholdConvergence>,
    ...
    py::class_<PSR, Strategy, std::shared_ptr<PSR>>(m, "PSR").def_readwrite("success_ratio",
                                                                            &PSR::success_ratio);
...
void define_population(py::module &main)
{
    py::class_<Population>(main, "Population")
        ...
        .def(py::init<Matrix, Matrix, Matrix, Vector, Matrix>(),
             py::arg("X"),
             py::arg("Z"),
             py::arg("Y"),
             py::arg("f"),
...
void define_constants(py::module &m)
{
    py::class_<constants_w>(m, "constants")
        .def_property_static(
            "cache_max_doubles",
            [](py::object) { return constants::cache_max_doubles; },
            [](py::object, size_t a) { constants::cache_max_doubles = a; })
        .def_property_static(
            "cache_min_samples",
            [](py::object) { return constants::cache_min_samples; },
            [](py::object, size_t a) { constants::cache_min_samples = a; })
        .def_property_static(
            "cache_samples",
            [](py::object) { return constants::cache_samples; },
            [](py::object, bool a) { constants::cache_samples = a; })
        .def_property_static(
            "clip_sigma",
            [](py::object) { return constants::clip_sigma; },
            [](py::object, bool a) { constants::clip_sigma = a; })
        .def_property_static(
            "use_box_muller",
            [](py::object) { return constants::use_box_muller; },
            [](py::object, bool a) { constants::use_box_muller = a; });
}

struct PyCriterion : restart::Criterion
{
    PyCriterion(const std::string &name) :
        restart::Criterion(name)
    {
    }
...
void define_restart_criteria(py::module &main)
{
    ...
    py::class_<TooMuchRepelling, Criterion, std::shared_ptr<TooMuchRepelling>>(m,
                                                                               "TooMuchRepelling")
        .def(py::init<>())
...
void define_restart_strategy(py::module &main)
{
    ...
    py::class_<IPOP, Strategy, std::shared_ptr<IPOP>>(m, "IPOP")
        // .def(py::init<Float, Float, Float>(), py::arg("sigma"), py::arg("dimension"),
        // py::arg("lamb"))
        .def_readwrite("ipop_factor", &IPOP::ipop_factor);

    py::class_<BIPOP, Strategy, std::shared_ptr<BIPOP>>(m, "BIPOP")
        // .def(py::init<Float, Float, Float, Float, size_t>(), py::arg("sigma"),
        // py::arg("dimension"), py::arg("lamb"), py::arg("mu"), py::arg("budget"))
...
void define_es(py::module &main)
{
    ...
    py::class_<OnePlusOneES, std::shared_ptr<OnePlusOneES>>(m, "OnePlusOneES")
        .def(py::init<size_t, Vector, Float, Float, size_t, Float, parameters::Modules>(),
             py::arg("d"),
             py::arg("x0"),
             py::arg("f0"),
             py::arg("sigma0") = 1.0,
    ...
    py::class_<MuCommaLambdaES, std::shared_ptr<MuCommaLambdaES>>(m, "MuCommaLambdaES")
        .def(py::init<size_t, Vector, Float, size_t, Float, parameters::Modules>(),
             py::arg("d"),
             py::arg("x0"),
             py::arg("sigma0") = 1.0,
             py::arg("budget") = 10'000,
