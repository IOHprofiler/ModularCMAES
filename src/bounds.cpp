...
static Float modulo2(const int x) { return (2 + x % 2) % 2; }
...
Vector BoundCorrection::delta_out_of_bounds(const Vector &xi,
                                            const Mask &oob,
...
Vector COTN::correct_x(const Vector &xi,
                       const Mask &oob,
                       const Float sigma,
                       const parameters::Settings &settings)
{
    ...
    return (oob).select(
        settings.lb.array() +
            settings.db.array() *
                ((y.array() > 0).cast<Float>() - (sigma * sampler().array().abs())).abs(),
        y);
}

Vector Mirror::correct_x(const Vector &xi,
                         const Mask &oob,
                         const Float sigma,
                         const parameters::Settings &settings)
{
    ...
    return (oob).select(
        settings.lb.array() +
            settings.db.array() *
                (y.array() - y.array().floor() - y.array().floor().unaryExpr(&modulo2)).abs(),
        y);
}

Vector UniformResample::correct_x(const Vector &xi,
                                  const Mask &oob,
                                  const Float sigma,
                                  const parameters::Settings &settings)
{
...
Vector Saturate::correct_x(const Vector &xi,
                           const Mask &oob,
                           const Float sigma,
                           const parameters::Settings &settings)
{
    const Vector y = delta_out_of_bounds(xi, oob, settings);
    return (oob).select(settings.lb.array() + settings.db.array() * (y.array() > 0).cast<Float>(),
                        y);
}

Vector Toroidal::correct_x(const Vector &xi,
                           const Mask &oob,
                           const Float sigma,
                           const parameters::Settings &settings)
{
