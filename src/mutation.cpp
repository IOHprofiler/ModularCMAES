...
namespace mutation
{
	...
	void CSA::mutate(FunctionType& objective, const size_t n_offspring, parameters::Parameters& p)
	{
		...
		for (Eigen::Index i = 0; i < static_cast<Eigen::Index>(n_offspring); ++i)
		{
			...
			{
				// TODO: Check if we can just break here otherwise
				p.pop.f(i) = objective(p.pop.X.col(i));
				p.stats.evaluations++;
				sequential_break_conditions = sq->break_conditions(i, p.pop.f(i), p.stats.global_best.y,
					p.settings.modules.mirrored);
			}
