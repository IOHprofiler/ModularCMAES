...
template <typename Callable> void call(Callable &o)
{
    static_assert(std::is_invocable_r_v<Float, Callable, Vector>,
                  "Incorrect objective function type");
...
Run run_modcma(parameters::MatrixAdaptationType mat_t, parameters::StepSizeAdaptation ssa)
{
    ...
    std::cout << "modcmaes: " << parameters::to_string(settings.modules.matrix_adaptation)
              << std::defaultfloat;
    ...
    if (rotated)
        std::cout << " (rotated)";
    const Float budget_used =
        static_cast<Float>(cma.p->stats.evaluations) / static_cast<Float>(budget) * 100;
    std::cout << "\nevals: " << cma.p->stats.evaluations << "/" << budget;
    std::cout << " ~ (" << std::defaultfloat << std::setprecision(3) << budget_used << "%)"
              << std::endl;
    ...
    return {(int)cma.p->stats.evaluations,
