...
namespace matrix_adaptation
{
    struct Adaptation
    {
        ...
        virtual void invert_mutation_steps(Population &pop) = 0;
    };
    ...
    struct None final : Adaptation
    {
        ...
        void invert_mutation_steps(Population& pop) override;
    };
    ...
    struct MatrixAdaptation final : Adaptation
    {
        Matrix M;
        Matrix M_inv;
        MatrixAdaptation(const size_t dim, const Vector &x0) : Adaptation(dim, x0, Vector::Ones(dim)), M(Matrix::Identity(dim, dim)), M_inv(Matrix::Identity(dim, dim)) {
        }
