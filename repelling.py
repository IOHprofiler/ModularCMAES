...
def plot(
    ...
    current_best_y = cma.p.stats.current_best.y
    global_best_y = cma.p.stats.global_best.y
    ...
    for t, tabu_point in enumerate(cma.p.repelling.archive, 1):
        ...
        current = Ellipse(
            ...
            *(2 * sigma * tabu_point.radius * np.diag(Ct)),
            ...
            facecolor="none",
            edgecolor="black",
            label=f"taboo point {t} radius: {tabu_point.radius: .2f} n_rep: {tabu_point.n_rep}",
            linewidth=2,
            linestyle="dashed",
            zorder=0,
        )
...
def interactive(fid=21, instance=6, dim=2, rep=True, coverage=5):
    lb = -5
    ub = 5
...
def collect(
    fid=21, dim=2, rep=True, n_instances=100, logged=True, coverage=5, budget_f=10_000
):
    if dim == 0:
        dims = range(2, 10)
    else:
