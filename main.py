...
def initialize_parameters(problem: ioh.ProblemType, budget: int, lambda_: int, mu_: int, n_generations: int = None, 
                        x0: np.ndarray = None, sigma0: float = 0.2, initialization_correction: str = None, 
                        svm: SVC = None, subpopulation_target: str = None, area_coefs: np.ndarray = None,
    ...
    CMAES_params = Parameters(d=problem.meta_data.n_variables, lambda_=lambda_, mu=mu_, x0=x0, initialization_correction=initialization_correction, 
                            svm=svm, subpopulation_target=subpopulation_target, area_coefs=area_coefs, #ub=problem.bounds.ub, lb=problem.bounds.lb, 
...
def initialize_centroids(problem: ioh.ProblemType, sub_pop: int, init_method: str) -> np.ndarray:
...
def initialize(problem_id: int, problem_instance: int, dimension: int, budget: int, iterations: int, lambda_: list[int], mu_: list[int], bound_corr: str, init_method: str, sigma0: float, init_corr: str, sharing_point: int, logger_info: dict):
    ...
    for i in range(subpop_n):
        params = initialize_parameters(problem=problem, budget=budget, lambda_=lambda_[i], mu_=mu_[i], x0=x0[i], sigma0=sigma0, bound_correction=bound_corr, initialization_correction=init_corr, svm=svm, subpopulation_target=labels[i], area_coefs=extract_svm_coefs(svm, labels, labels[i]))
        cmaes.append(modularcmaes.ModularCMAES(fitness_func=problem, parameters=params))

    return run_cma(CMAES=cmaes, iterations=iterations, corr=svm, sharing_point=sharing_point)

def run_cma(CMAES: list[modularcmaes.ModularCMAES], iterations: int, corr: SVC, sharing_point: int):
    ...
    for _ in range(iterations):
        for i in range(len(CMAES)):
            if corr: 
                ...
                CMAES[i].parameters.area_coefs = extract_svm_coefs(corr, labels, labels[i])
            break_conditions[i] = CMAES[i].step()
            # print(break_condition)
            if not break_conditions[i]: break
        
        if not any(break_conditions):
            break

        if corr:
            centroids = np.float64([CMAES[n].parameters.m.reshape((CMAES[i].parameters.d,)) for n in range(len(CMAES))])
            corr = SVC(kernel='linear').fit(centroids, labels)
    return CMAES

def main(problem_id: int, problem_instance: int, dimension: int, iterations: int, budget: int, lambda_: int, mu_: int, bound_corr: str, init_method: str, sigma0: float, init_corr: str, sharing_point: int, logger_info: dict):
    # initialize
    cmaes = initialize(problem_id=problem_id, problem_instance=problem_instance, dimension=dimension, budget=budget, iterations=iterations, lambda_=lambda_, mu_=mu_, bound_corr=bound_corr, init_method=init_method, sigma0=sigma0, init_corr=init_corr, sharing_point=sharing_point, logger_info=logger_info)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ic", "--init_corr", help="Initialization correction, currently only {None, 'svm'}", default=None, type=str)
